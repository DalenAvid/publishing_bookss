...
<body>
    ...
    <img src="{{ Storage::url(session('cover_image')) }}" alt="Обкладинка книги" style="max-width: 100%; height: auto;">
@else
    <p>Обкладинка книги не знайдена.</p>
@endif
<p>Шлях до обкладинки: {{ Storage::url(session('cover_image')) }}</p>

            {{-- <img src="{{ asset('storage/' . $coverImagePath) }}" alt="Обкладинка книги"> --}}
            {{-- @if (isset($cover_image))
                <img src="{{ $cover_image }}" alt="Обкладинка книги" style="max-width: 100%; height: auto;"> --}}
                ...
                {{-- @else
                <p>Обкладинка книги не знайдена.</p> --}}
                {{-- @if (session('book_data'))
                <img src="{{ session('book_data.cover_image') }}" alt="Обкладинка книги" style="max-width: 100%; height: auto;">
                <p>{{ session('book_data.title') }}</p>
            @else
                <p>Обкладинка книги або дані книги не знайдені.</p>
            @endif --}}
            ...
            {{-- <img src="{{ Storage::url($book['cover_image']) }}" alt="Обкладинка книги"> --}}
    ...
    var coverImage = "{{ session('cover_image') }}";
    if (coverImage) {
        var imgElement = document.getElementById('coverPreview');
        imgElement.src = "/storage/" + coverImage;
        imgElement.style.display = 'block';
    } else {
        console.log("Обкладинка не знайдена");
    }
});
